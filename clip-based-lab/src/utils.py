"""
Utility functions for CLIP-based deepfake detection.
"""
import os
import logging
...
def postprocess(
    ...
    for split in splits:
        if split in results_info and results_info[split] is not None:
            ...
            processed_keys.add(split)

    # Process custom folder (only if not already processed above)
    if folder_name is not None and folder_name in results_info and folder_name not in processed_keys:
